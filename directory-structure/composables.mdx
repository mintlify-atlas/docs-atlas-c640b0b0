...
Be aware that you have to run [`nuxt prepare`](/api/commands/prepare), [`nuxt dev`](/api/commands/dev) or [`nuxt build`](/api/commands/build) in order to let Nuxt generate the types.
...
You can access [plugin injections](/directory-structure/plugins#providing-helpers) from composables:
...
Nuxt only scans files at the top level of the [`app/composables/` directory](/directory-structure/composables), e.g.:
